...
\textbf{Лемма.} Собственные векторы унитарного оператора, соответствующие
 ...
 \section*{14. Нормальные операторы в эрмитовом пространстве, их собственные значения и собственные векторы.}
...
\Rightarrow (A^* - \overline{\lambda} \varepsilon)(x)=0 \Rightarrow A^*(x)=\overline{\lambda}x
\end{multline*}
...
\]
<x>- \( A \) инвариантно и  \( A^* \) инвариантно. \\
Рассмотрим 
\[
(x,A^*(y))\textbf{, где }y \in <x>^\perp; (A(x),y)=\lambda(x,y)=0\\ \textbf{ т.к. }y \in <x>^\perp;\Rightarrow
\]
\[
\Rightarrow A^*(y) \in <x>^\perp \Rightarrow 
\]
\(\Rightarrow <x>^\perp- A^*\) инвариантно\\
аналогично \(<x>^\perp - A\) инвариантно\\
\[
V =<x> \oplus <x>^\perp
\]
Если в \(<x>^\perp\) осталось только одно собственное значение, A уже диагональна в базисе \(<x>\) и \(<x>^\perp\).\\
иначе, повтрояя процедуру для \(<x>^\perp\) пока собственных значений не останется, получаем:\\
\[
V = \bigoplus_{i=1}^{n} \langle x_i \rangle
\]
\section*{15.Неотрицательно и положительно определённые симметрические операторы, извлечение корней.}
...
\) все \( \lambda > 0 \).\\
\textbf{Доказательство:}\\
положительно определённому A соответствует q(x) \(\Rightarrow\) q(x) приводиться к каноническому виду, где все коэф. >0 (например приведением к главным осям)
\[
...
\section*{16.Полярное разложение невырожденного линейного оператора в евклидовом пространстве.}
...
\section*{17.Тензоры как полилинейные функции. Примеры тензоров малых валентностей. Арифметические операции над тензорами, тензорное умножение. Тензорный базис и размерность пространства тензоров типа $(p; q)$.}
...
\]
*Валентность тензора- количество векторных и ковекторных аргументов тензора\\
...
\[
\mathbb{T}^1_1(V)\leftrightarrow MN_+(V)\leftrightarrow End(V);
\]\[
\mathbb{T}^1_1(V)\textbf{ это } End(V);
\]
6.(0,n)? - \( det()\)\\
Пусть \( V=F^n \Rightarrow det(x_1 ... x_n)\in F \Rightarrow det() \in \mathbb{T}^0_n(F^n)\)\\
...
\textbf{Теорема:}
\[
dim(\mathbb{T}^q_p(V))=n^{p+q}; \textbf{ при } dim(V)=n;
\]
\textbf{Доказательство:}\\
\[
...
каждая из (p+q) переменных независимо пробегает от 1 до n \Rightarrow \(dim(\mathbb{T}^q_p(V))=n^{p+q}\)\\
\textbf{Теорема Ейнштейна:}
Если пара индексов встречается снизу и сверху, то сумма по ним подразумевается, но не пишется.\\
Например:
\[
\sum^{n}_{i=1}x_ie_i=x^ie_i
\]
*объективно самая х**ня теорема ever
